...
##'
mkinmod.DFOP <- function(...,inpartri=c('default','water-sediment','advanced'),outpartri=c('default','water-sediment','advanced'),data=NULL,weight=NULL)
{## Example usage ##############################################################
 ## a <- mkinmod.full(
 ...
 spec <- lapply(1:ncompart,function(x){completeCompound_DFOP(spec[[x]],varname=obs_vars[x],first=(x==1),inpartri=inpartri,outpartri=outpartri,data=data,weight=weight)})
 names(spec) <- obs_vars
   ...
   if(inpartri=='default'){
     if(outpartri=='default'){
       ...
       for (target in to) {
         ...
         if(spec[[target]]$type=="DFOP"){## in this case, only one parent is possible, no children metabolites.
           fraction_to_target = paste('f',origin_box,'to', target, sep="_")
           fraction_not_to_target = paste("(1 - ", fraction_to_target, ")",
                                          sep="")
           if(is.null(fraction_left)) {
...
##' @author Zhenglei Gao
##' @export
completeCompound_DFOP <- function(compound=list(type='SFO',to='M1'),varname=NULL,first=FALSE,inpartri=c('default','water-sediment','advanced'),outpartri=c('default','water-sediment','advanced'),data=NULL,weight=NULL,update=NULL,...)
{
