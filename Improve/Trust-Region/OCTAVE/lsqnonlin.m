...
%   $Revision: 1.1.6.6 $  $Date: 2009/05/07 18:25:06 $
...
if nargin == 1
    if isa(FUN,'struct')
        ...
        error('optim:lsqnonlin:InputArg',['The input to LSQNONLIN should be either a ',...
...
switch funfcn{1}
    ...
    case 'fun_then_grad'
        ...
        try    
            ...
            optim_ME = MException('optim:lsqnonlin:InvalidFUN', ...
                'Failure in initial user-supplied Jacobian function evaluation. LSQNONLIN cannot continue.');
            userFcn_ME = addCause(userFcn_ME,optim_ME);
            rethrow(userFcn_ME)
        end
    otherwise
        error('optim:lsqnonlin:UndefCallType','Undefined calltype in LSQNONLIN.')
end
