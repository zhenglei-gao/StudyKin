parplot <- function(par,mkinmodini,eigen=FALSE,name=NULL, xlab = "Time", ylab = "Observed", legend = TRUE,pdf=TRUE,  err = NULL, weight = "none",scaleVar = FALSE,ctr=kingui.control(),...)
{
    ## example usage:
...
#################################################
    ...
    names(par) <-c( state.ini.optim.boxnames ,optim_parms)
    browser()
    odeini <-c(par[state.ini.optim.boxnames],state.ini.fixed )
    names(odeini) <-c(state.ini.optim.boxnames, state.ini.fixed.boxnames)
    odeparms <-c(par[optim_parms],parms.fixed)
#########################
outtimes = outtimes1
